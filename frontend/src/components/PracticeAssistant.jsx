...
export default function PracticeAssistant() {
  ...
  async function handleSend(text) {
    ...
    try {
      const res = await fetch(`${BACKEND}/api/stream`, {
        ...
        body: JSON.stringify({ message: msg, mode: 'practice', chat_history: [], thread_id: `${getThreadId()}_practice` }),
      })
      ...
      setMessages(prev => prev.map(m => m.id === aiId ? { ...m, content: 'Connection error. Is the backend running?' } : m))
    } finally {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', background: '#FFFFFF', fontFamily: FONT }}>

      {/* Header */}
      <div style={{ padding: '14px 18px', borderBottom: '1px solid rgba(0,0,0,0.07)', background: '#F7F7FA', flexShrink: 0, display: 'flex', alignItems: 'center', gap: '10px' }}>
        <div style={{ width: '28px', height: '28px', borderRadius: '8px', background: 'linear-gradient(135deg, #007AFF, #32ADE6)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '14px', flexShrink: 0 }}>✦</div>
        <div>
      ...
      {/* Messages */}
      <div style={{ flex: 1, overflowY: 'auto', padding: '14px 16px', display: 'flex', flexDirection: 'column', gap: '14px', scrollbarWidth: 'thin' }}>
        {messages.length === 0 && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.2 }}>
            <p style={{ fontSize: '13px', color: '#AEAEB2', marginBottom: '12px', lineHeight: 1.5 }}>
              Write some code and ask me to explain, debug, or improve it.
            </p>
            {SUGGESTIONS.map((s, i) => (
              <motion.button key={s} onClick={() => handleSend(s)}
                style={{ width: '100%', padding: '10px 12px', borderRadius: '10px', border: '1px solid rgba(0,0,0,0.08)', background: '#FFFFFF', color: '#3C3C43', fontSize: '13px', fontFamily: FONT, textAlign: 'left', cursor: 'pointer', marginBottom: '6px', display: 'block' }}
                initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 + i * 0.06 }}
                whileHover={{ borderColor: 'rgba(0,122,255,0.3)', background: 'rgba(0,122,255,0.02)' }}
              >
        ...
        <AnimatePresence initial={false}>
          {messages.map(msg => (
            ...
            >
              ...
              <div style={{
                maxWidth: '92%', padding: '10px 13px', fontSize: '13px', lineHeight: '1.65', fontFamily: FONT,
      ...
      <div style={{ padding: '10px 12px 14px', borderTop: '1px solid rgba(0,0,0,0.07)', background: '#F7F7FA', flexShrink: 0 }}>
        <div style={{ display: 'flex', alignItems: 'flex-end', gap: '8px', background: '#FFFFFF', borderRadius: '12px', padding: '8px 8px 8px 14px', boxShadow: '0 1px 4px rgba(0,0,0,0.07), 0 0 0 1px rgba(0,0,0,0.07)' }}>
          <textarea ref={inputRef} rows={1} value={input} onChange={e => setInput(e.target.value)}
            ...
            style={{ flex: 1, resize: 'none', border: 'none', outline: 'none', background: 'transparent', fontSize: '13px', fontFamily: FONT, lineHeight: '1.5', color: '#1C1C1E', minHeight: '22px', maxHeight: '100px', overflow: 'hidden' }}
          />
