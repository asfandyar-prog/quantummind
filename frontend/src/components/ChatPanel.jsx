...
export default function ChatPanel() {
  ...
  async function handleSend(text) {
    ...
    // Set both at once — prevents double render
...
function EmptyState({ mode, onSuggest }) {
  ...
  return (
    ...
    >
      <div style={{ width:'64px', height:'64px', borderRadius:'20px', background:`linear-gradient(135deg, ${meta.color}18, ${meta.color}30)`, border:`1px solid ${meta.color}22`, display:'flex', alignItems:'center', justifyContent:'center', fontSize:'28px', marginBottom:'20px' }}>⚛</div>
      <h2 style={{ fontSize:'22px', fontWeight:700, color:'#1C1C1E', marginBottom:'8px', fontFamily:FONT }}>Hello, Asfand 👋</h2>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'10px', width:'100%', maxWidth:'540px' }}>
        {suggestions.map((s, i) => (
          <motion.button key={s} onClick={() => onSuggest(s)}
            ...
            whileTap={{ scale:0.97 }}
          >
            <span style={{ color:meta.color, marginRight:'8px' }}>→</span>{s}
          </motion.button>
        ))}
      </div>
    </motion.div>
    ...
    <div style={{ display:'flex', flexDirection:'column', gap:'4px' }}>
      {lines.map((line, i) => {
        ...
        const html = line
          ...
          .replace(/`(.+?)`/g, `<code style="font-family:${MONO};font-size:13px;background:rgba(0,0,0,0.07);padding:1px 5px;border-radius:4px">$1</code>`)
...
function CodeBlock({ code }) {
  ...
  return (
    <div style={{ maxWidth:'90%', position:'relative' }}>
      <pre style={{ background:'#1A1A2E', color:'#CDD6F4', fontFamily:MONO, fontSize:'13px', lineHeight:'1.75', padding:'16px 20px', borderRadius:'12px', overflowX:'auto', margin:'4px 0 0' }}>
        {code}
      </pre>
